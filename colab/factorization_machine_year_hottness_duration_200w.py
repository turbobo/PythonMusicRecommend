# -*- coding: utf-8 -*-
"""Factorization Machine - year_hottness_duration_200w.ipynb
...
# songs = data[['song','year','artist_hotttnesss']]
songs = songs.rename(columns={'artist_hotttnesss': 'artistHotttnesss'})
# 根据songID去重
...
songs = songs.dropna(subset=['artistHotttnesss'])
songs = songs[songs.artistHotttnesss != 0]
print('artistHotttnesss***********************')
songs.info(verbose=True, max_cols=True, memory_usage=True, null_counts=True)
# 去重duration为空，为0
...
# 评分中year、artistHotttnesss为0的歌曲去除
print('ratings筛选前*********************************************************')
ratings.info(verbose=True, max_cols=True, memory_usage=True, null_counts=True)
...
df_final = ratings_ffm.groupby(["user", "song", "rating", 'year', 'artistHotttnesss','duration'])[ratings_ffm.columns.values[7:]].sum().reset_index()
df_final.info()
...
def split_testtrain(ratings, fraction):
    ...
    new_colnames = ['1_user', '2_movie', '0_rating','4_year','5_artistHotttnesss','6_duration']
